    ...
    this.hearthstoneRegistration = function () {
        ...
        try {
            return firebase.database().ref('/registration/' + eventId()).once('value').then(function (result) {
                ...
                if (!returnBool) {
                    var pushResult = firebase.database().ref('/registration/hearthstone/' + eventId()).push(self.hsObjectUnwrapped);
    ...
    this.leagueRegistration = function () {
        ...
        try {
            return firebase.database().ref('/registration/' + eventId()).once('value').then(function (result) {
                ...
                if (!returnBool) {
                    var pushResult = firebase.database().ref('/registration/league/' + eventId()).push(self.leagueObjectUnwrapped);
                    registrationId(pushResult.key);
                    confirmCheckout(true);
                }
            });
